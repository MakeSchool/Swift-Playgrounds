...
/*:
If you want to add a parameter, well you guessed it, it goes within the parantheses:
...
The compiler gives an error, becuase the function is telling the caller that it will return a String, but the function never actually used the "return" keyword to send something back. The compiler prevents this code to run to eventually crash your program!
*/


/*:
In order to take more than one parameters, you simply separate the inputs with a comma:
*/
...
In this function, we send in an array of integers as input, and expect 2 values to be returned. Notice the syntax: for the returned value, we have 2 items, first and last. They have their types declared, and they are wrapped inside parantheses. Furthermore, in order to return a tuple, the function simply wraps the items it wants to return inside parantheses.
*/
...
/*:
Let's see how a caller can use this function to receive 2 values back from the function:
*/
...
/*:
There is a totally different way to get back the values as well. You can define your own tuple items, and they will be bind to the values returned by the function. In the line below, item1 is bound to the value of the first item returned by the function. Note that they do not need to have their types defined. Why? because Swift is smart enough to realize that you are calling firstLast function which returns 2 integers, so as a programmer, you obviously wanted item1 and item2 to be integers as well. Cool huh?
*/
...
/*:
Alright, time to talk about an advanced topic: internal and external parameter names. Coming from another programming language, you probably have not heard about such a thing, but no worries, once you understand why it is needed, you will be using it everywhere in your code.
*/
...
/*:
When you define the external name for a parameter, you force the caller the use that name when they call the function:
*/
